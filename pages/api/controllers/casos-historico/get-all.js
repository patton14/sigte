...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (req.method === 'GET') {
            try {
                // Obtener los parámetros `page` y `size` de `req.query`
                const { page = 1, size = 50 } = req.query;  // Valores por defecto
                //Aqui se hace el llamado
                const result = await SigteAPICasosHistorico.get('/casos_historicos', {
                    params: { page, size }
                });
