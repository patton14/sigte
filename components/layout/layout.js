...
const Layout = (props) => {
    ...
    const getStatusSession = async() => {
        console.log(session?.resetPass)
        setEstado(session?.resetPass||null);
        setIsLoading(false)
        setLayoutState((prevLayoutState) => ({ ...prevLayoutState, staticMenuDesktopInactive: session?.resetPass || null }));
    }
    useEffect(() => {
        getStatusSession();
    }, [session?.user?.name]);
